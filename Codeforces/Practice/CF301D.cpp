...
namespace solution
{
    ...
    int mian()
    {
        ...
        for (int i = 1; i <= n; i++)
        {
            for (int j = 1; i * j <= n; j++)
            {
                p.push_back({b[i], b[i * j]});
            }
        }

        for (int i = 1; i <= m; i++)
        {
            int l = io.readi() - 1, r = io.readi();
            q.push_back({{l, l}, {i, 1}}), q.push_back({{l, r}, {i, -1}}), q.push_back({{r, l}, {i, -1}}), q.push_back({{r, r}, {i, 1}});
        }
        ...
        {
            while (j < p.size() && p[j].fi <= q[i].fi.fi) update(p[j++].se, 1);
            ans[q[i].se.fi] += query(q[i].fi.se) * q[i].se.se;
        }
