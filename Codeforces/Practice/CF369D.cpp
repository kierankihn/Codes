...
namespace solution
{
    ...
    int mian()
    {
        ...
        std::memset(dp, 0x3f, sizeof(dp)), dp[1][2] = 0;
        for (int i = 1; i <= n; i++)
        {
            for (int j = i + 1; j <= n + 1; j++)
            {
                if (sum[n] - sum[j - 1] != 0 && a[i] != 100)
                {
                    dp[j][j + 1] = std::min(dp[j][j + 1], dp[i][j] + 1);
                }
                if (max[j] != 100 && a[i] != 0)
                {
                    dp[i][j + 1] = std::min(dp[i][j + 1], dp[i][j] + 1);
                }
                if (sum[n] - sum[j - 1] != 0 && a[i] != 0)
                {
                    dp[j + 1][j + 2] = std::min(dp[j + 1][j + 2], dp[i][j] + 1);
                }
            }
        }
