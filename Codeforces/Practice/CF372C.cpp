...
namespace solution
{
    ...
    typedef pair<pair<i64, i64>, i64> p3l;
    ...
    int mian()
    {
        ...
        for (int i = 1; i <= m; i++)
        {
            i64 len = i == 1 ? 0 : d * (a[i].se - a[i - 1].se);

            std::deque<int> q;
            for (int j = 1; j <= len & j <= n; j++)
            {
                while (q.empty() == false && dp[i - 1 & 1][j] > dp[i - 1 & 1][q.back()]) q.pop_back();
                q.push_back(j);
            }
            for (int j = 1; j <= n; j++)
            {
                while (q.empty() == false && q.front() < j - len) q.pop_front();
                if (j + len <= n)
                {
                    while (q.empty() == false && dp[i - 1 & 1][j + len] > dp[i - 1 & 1][q.back()]) q.pop_back();
                    q.push_back(j + len);
                }
                dp[i & 1][j] = dp[i - 1 & 1][q.front()] + a[i].fi.se - std::abs(a[i].fi.fi - j);
            }
        }

        io.writeln(*std::max_element(dp[m & 1] + 1, dp[m & 1] + n + 1));
