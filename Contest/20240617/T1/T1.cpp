 ...
 * @author Kieran Kihn
 * @date 2024.06.17
 * @name P3521 [POI2011] ROT-Tree Rotations
...
namespace solution
{
    ...
    void upd(int &x, int o, int l = 1, int r = n)
    {
        ...
        if (l == r)
        {
            update(l, node[o].sum, x, l, r);
        }

        int mid = (l + r) >> 1;
        upd(node[x].lson, node[o].lson, l, mid), upd(node[x].rson, node[o].rson, mid + 1, r);
    ...
    void solve(int u)
    {
        ...
        i64 res = calc(root[son[u][0]], root[son[u][1]]);

        ans += std::min(res, (i64)node[root[son[u][0]]].sum * node[root[son[u][1]]].sum - res);

        upd(root[u] = root[son[u][0]], root[son[u][1]]);
    }
