...
namespace solution
{
    ...
    int n;
    i64 a[MAXN], minb[MAXN], maxb[MAXN];
    ...
    void dfs(int u)
    {
        ...
        i64 suma = 0, summaxb = 0, summinb = 0;
        for (auto v : e[u])
        {
            dfs(v), maxb[u] = std::min(maxb[u], maxb[v]), suma += a[v], summaxb += maxb[v], summinb += minb[v];
        }

        minb[u] = std::max(minb[u], -a[u]);
        minb[u] = std::max(minb[u], summinb + suma - a[u]);
        if (e[u].size() > 1)
        {
            maxb[u] = std::min(maxb[u], (a[u] - suma) / i64(e[u].size() - 1));
        }
        if (e[u].size() != 0 && (minb[u] > maxb[u] || summaxb + suma < a[u] + minb[u] || summinb + suma > a[u] + maxb[u]))
        {
            ans = 0;
        }
    }
