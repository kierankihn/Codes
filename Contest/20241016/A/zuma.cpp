...
namespace solution
{
    ...
    int mian()
    {
        std::freopen("zuma.in", "r", stdin);
        std::freopen("zuma.out", "w", stdout);

        io.read(n), scanf("%s", s + 1), io.reada(a + 1, a + n + 1);
        ...
        for (int len = 1; len <= n; len++)
        {
            for (int l = 1, r = len; r <= n; l++, r++)
            {
                for (int k = 0; k < l; k++)
                {
                    cmin(dp[l][r][k], dp[l + 1][r][0] + a[k + 1]);
                    for (int l_ = l + 1; l_ <= r; l_++)
                    {
                        if (s[l] == s[l_])
                        {
                            cmin(dp[l][r][k], dp[l_][r][k + 1] + dp[l + 1][l_ - 1][0]);
                        }
                    }
                }
            }
...
/*
    dp(l, r, i) <- dp(l + 1, r, 0) + cost(i + 1)
    dp(l, r, i) <- dp(l', r, i + 1) + dp(l + 1, l' - 1, 0)
*/