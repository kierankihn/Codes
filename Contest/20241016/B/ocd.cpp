...
namespace solution
{
    ...
    void dfs(int u)
    {
        ...
        {
            size++;
            int v = edge.fi, k = edge.se;
            if (color[v] == -1)
            {
                color[v] = (color[u] + k) % 3, dfs(v);
            }
            else
            {
                if (color[v] != (color[u] + k) % 3)
                {
                    flag = true;
                }
            }
    ...
    int mian()
    {
        std::freopen("ocd.in", "r", stdin);
        std::freopen("ocd.out", "w", stdout);

        io.read(n, m);
        for (int i = 1; i <= m; i++)
        {
            int u = io.readi(), v = io.readi();
            e[u].push_back({v, 1}), e[v].push_back({u, 2});
        }
        ...
        for (int i = 1; i <= n; i++)
        {
            if (color[i] == -1)
            {
                color[i] = 0, cnt[0] = cnt[1] = cnt[2] = 0, flag = false, size = 0;
