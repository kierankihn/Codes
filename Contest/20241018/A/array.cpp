...
namespace solution
{
    ...
    int mian()
    {
        ...
        i128 x, y, gcd = exgcd(p, q, x, y), lcm = 1LL * p * q / gcd;
        ...
        for (int i = 1; i <= n; i++)
        {
            ...
            {
                i128 x_ = x * a[i] / gcd, y_ = y * a[i] / gcd;

                x_ += y_ / (lcm / q) * lcm / p, y_ %= lcm / q;

                ans += std::min({abs(x_) + abs(y_), abs(x_ - q / gcd) + abs(y_ + p / gcd), abs(x_ + q / gcd) + abs(y_ - p / gcd)});
            }
        }
