...
namespace solution
{
    ...
    int mian()
    {
        std::freopen("strategy.in", "r", stdin);
        std::freopen("strategy.out", "w", stdout);

        io.read(n, k);
        for (int i = 1; i <= n; i++)
        {
            c[i] = (a[i] = io.readi()) - (b[i] = io.readi()), ans += b[i];
        }
        ...
        {
            i64 res = ans;
            if (a[i] - b[i] < 0 && a[i] - b[i] <= c[k])
            {
                res = std::min(res, ans - a[i] + std::min(c[k + 1], 0LL));
                if (a[i] - b[i] == c[k + 1])
                {
                    res = std::min(res, ans - b[i]);
                }
            }
            else
            {
