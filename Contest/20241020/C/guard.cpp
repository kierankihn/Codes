...
namespace solution
{
    ...
    constexpr int DELTA = 2e2;
    constexpr int MAXN  = 2e2 + 5;

    int n, m, p;
    int a[MAXN], b[MAXN];

    dbi dp[MAXN][MAXN][MAXN * 2];
    ...
    int mian()
    {
        ...
        for (int i = 0; i < n; i++)
        {
            for (int j = 0; j <= i; j++)
            {
                for (int k = -i; k <= n; k++)
                {
                    dp[i + 1][j][k + DELTA] += dp[i][j][k + DELTA] * (100 - b[i + 1]) / 100.00;
                    dp[i + 1][j + 1][std::min(k + a[i + 1], n) + DELTA] += dp[i][j][k + DELTA] * b[i + 1] / 100.00;
                }
            }
        }
