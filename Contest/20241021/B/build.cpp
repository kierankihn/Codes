...
namespace solution
{
    ...
    int solve(std::function<bool(p3i, p3i)> cmp)
    {
        int res = 0;
        ...
        for (auto e : edge)
        {
            ...
            {
                merge(e.se.fi, e.se.se), res += e.fi;
            }
        }
    ...
    int mian()
    {
        ...
        int min = solve(std::less<p3i>()), max = solve(std::greater<p3i>());

        for (int i = 1; i <= q; i++)
        {
            int x = io.readi();
            io.writeln(min <= x && x <= max ? "Yes" : "No");
        }
