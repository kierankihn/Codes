...
namespace solution
{
    ...
    int mian()
    {
        std::freopen("firework.in", "r", stdin);
        std::freopen("firework.out", "w", stdout);
        ...
        for (int i = 1; i <= n; i++)
        {
            cmax(ans, dp[i][0][0] = std::max({dp[i - 1][0][0], dp[i - 1][0][1], 0LL}) + a[i]);
            for (int j = 1; j <= m; j++)
            {
                dp[i][j][0] = std::max({dp[i - 1][j][0], dp[i - 1][j][1], 0LL}) + a[i];
                dp[i][j][1] = std::max({dp[i - 1][j - 1][0], dp[i - 1][j][1], 0LL}) + a[i] * w;

                ans = std::max({ans, dp[i][j][0], dp[i][j][1]});
            }
        }
