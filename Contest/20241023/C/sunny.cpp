...
namespace solution
{
    ...
    constexpr i64 INF  = 0x3f3f3f3f3f3f3f;
    ...
    void init()
    {
        ...
        for (int i = 1; i <= N; i++)
        {
            for (int j = 1; i * j <= N; j++)
            {
                min[i * j] = std::min(min[i * j], (i + j) * 2LL);
            }
        }
    }
    ...
    int mian()
    {
        std::freopen("sunny.in", "r", stdin);
        std::freopen("sunny.out", "w", stdout);

        init();

        i64 sqrtn = std::sqrt(n = io.readi());

        for (int i = 0; i <= N && i < sqrtn; i++)
        {
            i64 x = sqrtn - i, y = (n - 1) / x;

            if (n - x * y <= N)
            {
                ans = std::min(ans, (x + y) * 2 + min[n - x * y]);
            }
        }
