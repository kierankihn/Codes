...
namespace solution
{
    ...
    i64 dis[2][MAXN], cnt[2][MAXN], vis[2][MAXN];
    void dijkstra(int x, i64 *dis, i64 *cnt, i64 *vis)
    {
        std::priority_queue<pll, std::vector<pll>, std::greater<>> q;
        q.push({dis[x] = 0, x}), cnt[x] = 1;
        ...
        while (q.empty() == false)
        {
            ...
            for (auto edge : e[now.se])
            {
                i64 u = now.se, v = edge.fi, w = edge.se;
                if (dis[u] + w < dis[v])
                {
                    q.push({dis[v] = dis[u] + w, v}), cnt[v] = 0;
                }
                if (dis[u] + w == dis[v])
                {
    ...
    int mian()
    {
        std::freopen("reunion.in", "r", stdin);
        std::freopen("reunion.out", "w", stdout);
        ...
        dijkstra(s, dis[0], cnt[0], vis[0]);
        dijkstra(t, dis[1], cnt[1], vis[1]);

        for (int u = 1; u <= n; u++)
        {
            if (dis[0][u] + dis[1][u] == dis[0][t] && dis[0][u] == dis[1][u])
            {
                ans += (cnt[0][u] * cnt[1][u] % MOD) * (cnt[0][u] * cnt[1][u] % MOD) % MOD, ans %= MOD;
            }
            for (auto edge : e[u])
            {
                i64 v = edge.fi, w = edge.se;
                if (dis[0][u] * 2 < dis[0][t] && dis[1][v] * 2 < dis[0][t] && dis[0][u] + dis[1][v] + w == dis[0][t])
                {
                    ans += (cnt[0][u] * cnt[1][v] % MOD) * (cnt[0][u] * cnt[1][v] % MOD) % MOD, ans %= MOD;
                }
            }
        }

        io.writeln(cnt[0][t], cnt[1][s], ans);
        io.writeln((cnt[0][t] * cnt[1][s] % MOD - ans + MOD) % MOD);
