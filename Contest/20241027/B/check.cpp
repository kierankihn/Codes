...
namespace solution
{
    ...
    i64 b[MAXN][MAXN], c[MAXN][MAXN], d[MAXN][MAXN];

    i64 ans = 0;

    int mian()
    {
        std::freopen("check.in", "r", stdin);
        std::freopen("check.out", "w", stdout);
        ...
        for (int i = 1; i <= n; i++)
        {
            ...
            for (int j = 1; j <= m; j++)
            {
                a[i][j] = s[j - 1] == '1' ? a[i - 1][j] + 1 : 0;
                b[i][j] = 1LL * i * i * j * j % MOD;
                c[i][j] = c[i - 1][j] + c[i][j - 1] - c[i - 1][j - 1] + b[i][j], c[i][j] %= MOD;
                d[i][j] = d[i][j - 1] + c[i][j], d[i][j] %= MOD;
            }
        }

        for (int i = 1; i <= n; i++)
        {
            std::stack<pii> stk = std::stack<pii>({{0, 0}});
            for (int j = 1; j <= m + 1; j++)
            {
                int len = 0;
                while (a[i][stk.top().fi] > a[i][j])
                {
                    ans += d[a[i][stk.top().fi]][len += stk.top().se], ans %= MOD, stk.pop();
                    ans -= d[std::max(a[i][stk.top().fi], a[i][j])][len] - MOD, ans %= MOD;
                }
                stk.push({j, ++len});
            }
        }
