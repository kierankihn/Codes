...
namespace solution
{
    ...
    int mian()
    {
        std::freopen("godluoskyzheng.in", "r", stdin);
        std::freopen("godluoskyzheng.out", "w", stdout);

        io.read(n);
        for (int i = 1; i <= n; i++)
        {
            sum[i] = sum[i - 1] + (a[i] = io.readi());
        }
        ...
        if (n > 300)
        {
            ...
            for (int i = 1; i < n; i++)
            {
                for (int j = i + 1; j < n; j++)
                {
                    ans = std::max(ans, sum[i] * (sum[j] - sum[i]) + (sum[j] - sum[i]) * (sum[n] - sum[j]));
                }
            }
            io.writeln(ans);
        ...
        for (int i = 1; i <= n; i++)
        {
            ...
            for (int j = 1; j < i; j++)
            {
                for (int k = 0; k < j; k++)
                {
                    dp[i][j] = std::max(dp[i][j], dp[j][k] + (sum[i] - sum[j]) * (sum[j] - sum[k]));
                }
            }
        }

        io.writeln(*std::max_element(dp[n], dp[n] + n + 1));
