...
namespace solution
{
    ...
    int mian()
    {
        std::freopen("eat.in", "r", stdin);
        std::freopen("eat.out", "w", stdout);
        ...
        while (t--)
        {
            ...
            std::memset(dp, 0xbf, sizeof(dp)), dp[k][k] = 0;

            for (int len = 2; len <= n; len++)
            {
                for (int l = 1, r = len; r <= n; l++, r++)
                {
                    dp[l][r] = std::max(dp[l][r - 1] + a[r] >= 0 ? dp[l][r - 1] + a[r] : -INF, dp[l + 1][r] + a[l + 1] >= 0 ? dp[l + 1][r] + a[l + 1] : -INF);
                }
            }

            io.writeln(dp[1][n] >= 0 ? "Yes" : "No");
        }
