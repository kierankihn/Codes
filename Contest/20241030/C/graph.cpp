...
namespace solution
{
    ...
    std::set<std::basic_string<int>> s[2];

    int vis[2][MAXN];
    std::deque<int> q;
    void dfs(int u, int color, int fa)
    {
        vis[color][u] = true, q.push_back(u);

        for (auto v : e[color][u])
        {
            if (v != fa)
            {
                for (int i = 1; i <= q.size() && i <= 3; i++)
                {
                    if (i == q.size())
                    {
                        int flag = false;
                        for (auto v_ : e[color][v])
                        {
                            if (v_ != u && v_ == q.front())
                            {
                                flag = true;
                    ...
                    std::basic_string<int> x = {v};
                    for (int j = 1; j <= i; j++)
                    {
                        x.push_back(q[q.size() - j]);
                    }
    ...
    int mian()
    {
        ...
        for (int i = 1; i <= m; i++)
        {
            ...
            deg[w][a[i] = u]++, deg[w][b[i] = v]++, c[i] = w;
        }

        for (int i = 1; i <= m; i++)
        {
            if (deg[c[i]][a[i]] <= 2 && deg[c[i]][b[i]] <= 2)
            {
                e[c[i]][a[i]].push_back(b[i]), e[c[i]][b[i]].push_back(a[i]);
            }
        }
