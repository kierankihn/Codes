...
namespace solution
{
    ...
    constexpr int MAXLOGN = 60 + 5;

    struct Matrix
    {
        static constexpr int SIZE = 101;
        std::array<std::array<i64, SIZE>, SIZE> val;
    ...
    Matrix operator*(const Matrix &a, const Matrix &b)
    {
        ...
        for (int i = 1; i < Matrix::SIZE; i++)
        {
            for (int j = 1; j < Matrix::SIZE; j++)
            {
                ...
                {
                    res.val[i][k] += a.val[i][j] * b.val[j][k];
                    res.val[i][k] %= MOD;
                }
            }
        }
    ...
    Matrix trans[MAXLOGN], f[MAXLOGN], g[MAXLOGN];

    Matrix ans;

    int mian()
    {
        std::freopen("dragon.in", "r", stdin);
        ...
        f[0] = trans[0], init(g[0]);
        for (int i = 1; i <= 60; i++)
        {
            f[i] = (g[i] = f[i - 1] * g[i - 1]) * trans[i];
        }
