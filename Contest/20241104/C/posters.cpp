...
namespace solution
{
    ...
    int sx[MAXN], sy[MAXN], ex[MAXN], ey[MAXN], d[MAXN][MAXN];
    int calc()
    {
        ...
        {
            if (sx[id] < 1 || sx[id] > n || sy[id] < 1 || sy[id] > m)
            {
                continue;
            }

            if (ex[id] < 1 || ex[id] > n || ey[id] < 1 || ey[id] > m)
            {
                continue;
            }

            for (int i = sx[id]; i <= ex[id]; i++)
            {
                for (int j = sy[id]; j <= ey[id]; j++)
                {
                    d[i][j] = 1;
                }
            }
    ...
    int mian()
    {
        std::freopen("posters.in", "r", stdin);
        std::freopen("posters.out", "w", stdout);
        ...
        if (k == 2)
        {
            ans = a[1] * b[1] + a[2] * b[2] - std::max(0, a[1] + a[2] - n) * std::max(0, b[1] + b[2] - m);
        }
        if (k == 3)
        {
            for (int i = 1; i <= 3; i++) c[i] = i;
            do
            {
                sx[c[1]] = 1, sy[c[1]] = 1;
                for (sx[c[2]] = 1, ex[c[2]] = a[c[2]]; ex[c[2]] <= n; sx[c[2]]++, ex[c[2]]++)
                { 
                    for (sy[c[2]] = 1, ey[c[2]] = b[c[2]]; ey[c[2]] <= m; sy[c[2]]++, ey[c[2]]++)
                    {
                        for (sx[c[3]] = 1, ex[c[3]] = a[c[3]]; ex[c[3]] <= n; sx[c[3]]++, ex[c[3]]++)
                        {
                            for (sy[c[3]] = 1, ey[c[3]] = b[c[3]]; ey[c[3]] <= m; sy[c[3]]++, ey[c[3]]++)
                            {
            ...
            } while (std::next_permutation(c + 1, c + 4));
        }
        if (k >= 4)
        {
            for (int i = 1; i <= k; i++)
