...
namespace solution
{
    ...
    int n, m, q;
    i64 a[MAXN], b[MAXN], sum[MAXN];

    std::vector<int> p[MAXN];

    int mian()
    {
        std::freopen("ring.in", "r", stdin);
        std::freopen("ring.out", "w", stdout);
        ...
        if (m == 1)
        {
            ...
            for (int i = 1; i <= q; i++)
            {
                ...
                if (op == 1)
                {
                    ...
                    io.writeln(sum[pos + r] - sum[pos + l - 1]);
                }
                if (op == 2)
                {
                    if (io.readi() == 1)
        ...
        else
        {
            for (int i = 1; i <= q; i++)
            {
                ...
                {
                    int x = io.readi();
                    for (int j = int(p[x].size()) - 1; j >= 1; j--)
                    {
                        int prev = p[x][j - 1], next = p[x][j];
                        std::swap(a[prev], a[next]);
                    }
                }
            }
        }
