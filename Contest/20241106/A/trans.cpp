...
namespace solution
{
    ...
    i64 query(i64 x)
    {
        ...
        if (up != s.end())
        {
            res = std::min(res, std::abs(x + *up));
        }
        if (up != s.begin())
        {
            res = std::min(res, std::abs(x + *std::prev(up)));
        }
    ...
    int mian()
    {
        std::freopen("trans.in", "r", stdin);
        std::freopen("trans.out", "w", stdout);
        ...
        for (int i = 1; i <= m; i++)
        {
            if (i > n)
            {
                c[i] = c[i - 1] * -1 + b[i] * (n & 1 ? -1 : 1) - b[i - n];
            }
            else
            {
                c[i] = c[i - 1] + b[i] * (i & 1 ? -1 : 1);
            }
        ...
        io.writeln(query(sum));
        for (int i = 1; i <= q; i++)
        {
            int l = io.readi(), r = io.readi(), v = io.readi();
            if ((std::max(l, r) - std::min(l, r) + 1) & 1)
            {
                sum += v * (r & 1 ? 1 : -1);
            }
