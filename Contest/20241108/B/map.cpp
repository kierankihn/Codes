...
namespace solution
{
    ...
    i64 min[MAXN][2], id[MAXN][2];
    void update(int x, i64 v, int p)
    {
        if (v < min[x][0])
        {
            if (p != id[x][0])
            {
                min[x][1] = min[x][0], id[x][1] = id[x][0];
            }
            min[x][0] = v, id[x][0] = p;
        }
        else if (v < min[x][1] && p != id[x][0])
        {
            min[x][1] = v, id[x][1] = p;
        }
    }
    ...
    void dfs(int u)
    {
        ...
        ans = std::min(ans, min[u][0] + min[u][1]);

        for (auto [v, w] : e[u])
        {
            update(v, min[u][0] + w, id[u][0]);
            update(v, min[u][1] + w, id[u][1]);
            if (vis[v] == false)
            {
                dfs(v);
            }
            update(u, min[v][0] + w, id[v][0]);
            update(u, min[v][1] + w, id[v][1]);
        }
    }
