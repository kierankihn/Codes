...
namespace solution
{
    ...
    chr s[MAXN];
    int a[MAXN];
    i64 sum[MAXN], pow[MAXN];
    void input()
    {
        fin >> (s + 1), n = std::strlen(s + 1);
        for (int i = 1; i <= n; i++)
        {
            a[i] = s[i] - '0';
        }
    ...
    int dp[MAXN][MAXC], cnt[MAXN][MAXC][MAXC], cnt_p[MAXC];
    void mod(i64 &x)
    {
        x = (x >= MOD ? x - MOD : x);
    }
    i64 solve(int pos, int x, int flag)
    {
        if (flag == false && dp[n - pos + 1][x] != -1) return dp[n - pos + 1][x];
        if (pos == n + 1)
        {
            if (flag) std::fill(cnt_p, cnt_p + 10, 0);
            return 0;
        }
        ...
        i64 res = 0;
        for (int i = 0; i <= (flag ? a[pos + 1] : 9); i++)
        {
            mod(res += solve(pos + 1, i, flag && i == a[pos + 1]));
            if (flag && i == a[pos + 1])
            {
                for (int j = 0; j <= 9; j++)
                {
        ...
        mod(cnt_n[x] += flag ? sum[pos + 1] : pow[n - pos]);

        for (int i = 0; i <= 9; i++)
        {
            (flag ? cnt_p : cnt[n - pos + 1][x])[i] = cnt_n[i];
        }

        return flag ? res : (dp[n - pos + 1][x] = res);
    }
    ...
    i64 solve(bool flag)
    {
        ...
        for (int i = n; i >= 1; i--)
        {
            sum[i] = sum[i + 1] + (a[i]) * pow[n - i], sum[i] %= MOD;
        }

        std::fill(cnt_p, cnt_p + 10, 0);
    ...
    int mian()
    {
        ...
        while (t--)
        {
            fout << (MOD - solve(true) + solve(false)) % MOD << '\n';
        }
