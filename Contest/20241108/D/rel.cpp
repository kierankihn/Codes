...
namespace solution
{
    ...
    i64 solve()
    {
        std::priority_queue<p3l> q;
        for (int u = 1; u <= n; u++)
        {
            for (int v = u + 1; v <= n; v++)
            {
                q.push({i64((a[u] & a[v]).count()), {u, v}});
            }
        }
        ...
        while (q.empty() == false)
        {
            if (find(q.top().se.fi) != find(q.top().se.se))
            {
                res += q.top().fi;
                merge(q.top().se.fi, q.top().se.se);
            }
            q.pop();
        }
    ...
    int mian()
    {
        std::freopen("rel.in", "r", stdin);
        std::freopen("rel.out", "w", stdout);
