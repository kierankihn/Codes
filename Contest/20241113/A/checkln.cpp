...
namespace solution
{
    ...
    int mian()
    {
        std::freopen("checkln.in", "r", stdin);
        std::freopen("checkln.out", "w", stdout);
        ...
        {
            map[a[i]] = i;
            if (map.count(a[i] - k)) merge(i, map[a[i] - k]);
            if (map.count(a[i] + k)) merge(i, map[a[i] + k]);
            res[i] = ++ans;
        }

        io.writealn(res + 1, res + n + 1);
