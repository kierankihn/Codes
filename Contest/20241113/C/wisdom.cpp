...
namespace solution
{
    ...
    i64 solve(i64 k1, i64 k2)
    {
        i64 suma = 0, sumb = 0;

        std::fill(p, p + MAXLOGV, 0);
        std::sort(a + 1, a + n + 1, [&](const p3i &a, const p3i &b)
                  { return a.se.fi * k1 + a.se.se * k2 < b.se.fi * k1 + b.se.se * k2; });
        for (int i = 1; i <= n; i++)
        {
            if (insert(a[i].fi)) suma += a[i].se.fi, sumb += a[i].se.se;
        }
    ...
    int mian()
    {
        std::freopen("wisdom.in", "r", stdin);
        std::freopen("wisdom.out", "w", stdout);
        ...
        for (int i = 1; i <= n; i++)
        {
            for (int j = i + 1; j <= n; j++)
            {
                ans = std::min(ans, solve(a[j].se.se - a[i].se.se, a[i].se.fi - a[j].se.fi));
                ans = std::min(ans, solve(a[j].se.se - a[i].se.se - 1, a[i].se.fi - a[j].se.fi));
                ans = std::min(ans, solve(a[j].se.se - a[i].se.se + 1, a[i].se.fi - a[j].se.fi));
            }
        }
