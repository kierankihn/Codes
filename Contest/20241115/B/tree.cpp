...
namespace solution
{
    ...
    void solve(int u, int fa)
    {
        ...
        for (auto v : e[u])
        {
            if (v != fa)
            {
                ...
                std::memcpy(dp[0], dp[u], sizeof(dp[u]));
                std::memset(dp[u], 0, sizeof(dp[u]));
                for (int i = 0; i <= 3; i++)
                {
                    for (int j = 0; j <= 3 && i + j <= 4; j++)
                    {
                        if (i + j <= 3) dp[u][i + j][0] += dp[0][i][0] * (dp[v][j][0] + dp[v][j][1] + dp[v][j][2]) % MOD;
                        if (i + j <= 3) dp[u][i + j][1] += dp[0][i][1] * (dp[v][j][0] + dp[v][j][1] + dp[v][j][2]) % MOD;
                        if (i + j <= 3) dp[u][i + j][2] += dp[0][i][2] * (dp[v][j][0] + dp[v][j][1] + dp[v][j][2]) % MOD;

                        if (i + j <= 3) dp[u][i + j][1] += dp[0][i][0] * dp[v][j][1] % MOD;
                        if (i + j <= 3) dp[u][i + j][2] += dp[0][i][1] * dp[v][j][0] % MOD;
                        if (i + j + 1 <= 3) dp[u][i + j + 1][1] += dp[0][i][0] * dp[v][j][0] % MOD;
                        if (i + j - 1 >= 0) dp[u][i + j - 1][2] += dp[0][i][1] * dp[v][j][1] % MOD;
                    }
                }
            }
        }
    ...
    int mian()
    {
        ...
        io.writeln((dp[1][3][0] + dp[1][3][1] + dp[1][3][2]) % MOD);
