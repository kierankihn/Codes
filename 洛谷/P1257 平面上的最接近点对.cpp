...
namespace solution
{
    ...
    i64 calc(const pii &a, const pii &b)
    {
    ...
    void solve(int l, int r)
    {
        ...
        i64 mid = (l + r) >> 1, h = p[mid].first;
        solve(l, mid), solve(mid + 1, r);

        std::sort(p + l, p + r + 1);
        auto begin = std::lower_bound(p + l, p + r + 1, std::make_pair(h - i64(std::ceil(std::sqrt(ans))), -INF)),
             end   = std::upper_bound(p + l, p + r + 1, std::make_pair(h + i64(std::ceil(std::sqrt(ans))), INF));
